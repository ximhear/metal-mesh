...
public actor ModelIOQueue {
    public static let shared = ModelIOQueue()

    public func run<T: Sendable>(_ body: @Sendable () throws -> T) throws -> T {
        try body()
    }

    public func run<T: Sendable>(_ body: @Sendable () -> T) -> T {
        body()
    }
}
