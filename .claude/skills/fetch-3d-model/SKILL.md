...
- **노멀 맵은 PNG로**: `usd` include 목록의 `nor_gl` EXR은 ImageIO가 디코딩하지 못한다(압축 방식 미지원). `files` 응답의
  `nor_gl.1k.png.url`을 `textures/`에 받고 EXR은 지운다. **다중 재질 에셋**(예: brass_goblets)은 최상위 키가
  `<재질>_nor_gl`, `<재질>_rough`, `<재질>_metal`처럼 재질별로 나뉘어 있고 usd include에 rough/metal이 빠져 있을 수 있으니
  재질마다 `_nor_gl`(png), `_rough`(jpg), `_metal`(jpg)을 따로 받는다. 파일명은 `<재질>_<종류>_1k.<ext>`라 앱 로더의 이웃 파일 규칙에 맞는다.
